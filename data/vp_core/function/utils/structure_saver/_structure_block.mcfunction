#vp_core:utils/structure_saver/_structure_block
# 生成保存模式的结构方块
# 若field_index属于0到总数量-1，则结构方块框选当前field_index指向的结构
# 若field_index等于总数量，则结构方块框选用于保存元信息的decorated_pot
# 占用temp{<temp_x,int>,<temp_y,int>,<temp_z,int>,<temp_l,int>,<temp_s,int>,<temp_h,int>,<temp_area,int>,<temp_volume,int>,<temp_i,int>}
# 占用[storage math:io xyz,ListDouble,3]
# 占用[storage vp_core:io temp,Compound]
# 输出结构方块（位于field_center沿z-方向前进field_size*24+3格的位置）
# 输出用于保存元信息的decorated_pot（位于结构方块上方一格）
# 需要传入世界实体
...
data modify storage vp_core:io temp set value {posX:0,posY:0,posZ:0}
execute store result storage vp_core:io temp.posX int 1 run scoreboard players get temp_x int
execute store result storage vp_core:io temp.posY int 1 run scoreboard players get temp_y int
execute store result storage vp_core:io temp.posZ int 1 run scoreboard players get temp_z int
# 生成结构方块
execute at @s run setblock ~ ~ ~ minecraft:air
execute at @s run setblock ~ ~ ~ minecraft:structure_block{mode:"SAVE",sizeX:48,sizeY:48,sizeZ:48}
# 设置保存路径
execute at @s run function vp_core:utils/structure_saver/macro_path with storage vp_core:io {}
# 保存元信息特殊设置
execute if score temp_i int = temp_volume int run data modify storage vp_core:io temp set value {posX:0,posY:1,posZ:0,sizeX:1,sizeY:1,sizeZ:1}
execute at @s run data modify block ~ ~ ~ {} merge from storage vp_core:io temp

# 生成元信息陶罐
execute if score temp_i int = temp_volume int at @s run function vp_core:utils/structure_saver/decorated_pot with storage vp_core:io {}
