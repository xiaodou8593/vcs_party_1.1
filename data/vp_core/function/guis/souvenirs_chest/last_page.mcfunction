#vp_core:guis/souvenirs_chest/last_page
...
execute if score temp_cur int matches ..1 at @s run playsound minecraft:block.note_block.bass player @a ~ ~ ~ 1.0 1.0
execute if score temp_cur int matches ..1 run return run scoreboard players set update_gui int 1
...
execute store result storage vp_core:io result.cur int 1 run scoreboard players remove temp_cur int 1
